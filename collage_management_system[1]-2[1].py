...
(code TEXT PRIMARY KEY NOT NULL, name TEXT NOT NULL, hours INTEGER NOT NULL)""")
...
def ADD_STUDENT():
    # name
    n = input("Enter full_name of student : ").capitalize()
    # id
    ...
    # level
    l = input("Enter level of student(1/2/3/4/5/6/7), if more write else : ").lower()
    if l == "else":
        l = input("Enter level of student : ")
    # password
    ...
    if cursor.fetchone() != None:
        password = input("Error:this password not available, Enter a different password :")
...
def REMOVE_STUDENT():
    ...
    while True:
        ...
        if cursor.fetchone() is None:
            id = input("Sorry this id not exist in database, enter an available ID :")
        else:
            break
    cursor.execute("select rowid from students where id=?", (id,))
    row = cursor.fetchone()
...
def EDIT_STUDENT_INFORMATION():
    ...
    while True:
        ...
        row = cursor.fetchone()
        if row is None:
            id = input("Sorry this id not exists in database, enter an available ID :")
        else:
            break
    row_id = row[0]
    print(
    ...
    while end:
        ...
        if choice == '1':
            n = input("Enter new full name of student  : ").capitalize()
            flag_e = True
        elif choice == '2':
            id = input("Enter ID of student  : ")
            while True:
            ...
            # level
            l = input("Enter new level of student(1/2/3/4/5/6/7), if more write else : ").lower()
            if l == "else":
                l = input("Enter level of student : ")
            flag_e = True
        elif choice == '4':
            # password
            p = input("Enter new password of student :")
            flag_e = True
        elif choice == '5':
            # GPA of student
            GPA = float(input("Enter new GPA of student : "))
            # check GPA from 0 to 4 only
            while not (0 <= GPA <= 4):
                GPA = float(input("Please, enter GPA from 0 to 4 only: "))
            flag_e = True
            ...
            # group
            g = input("Enter group of student, if there is no group press enter: ").capitalize()
            if g is None:
                g = ' '
            flag_e = True
        elif choice == '7':
        ...
        if flag_e:
            stu = (n, id, l, p, GPA, g)
            cursor.execute(f"UPDATE students SET name = ?, id = ?, level = ?, password = ?, GPA = ?, group_ = ? where rowid = {row_id}", stu)
            print("Student information \"edited\" successfully")
            print()
            cursor.execute(f"SELECT * FROM students WHERE rowid = '{row_id}'")
            data = cursor.fetchall()
            for k in data:
                print(f"""edited information of student:
...
def ADD_COURSES():
    ...
    n = input("Enter the name of the course: ").lower()
...
def SEE_ALL_INFORMATION_OF_A_STUDENT():
    while True:
        ...
        cursor.execute(f"SELECT id FROM students WHERE id = '{id}'")
        if cursor.fetchone() is None:
            print("Sorry this id not exist in database")
        else:
            break
    cursor.execute(f"SELECT * FROM students WHERE id = '{id}'")
    print("")
    data = cursor.fetchall()
    for row in data:
        print(f"""All information of a student:
(full name => {row[0]}, stu_id => {row[1]}, level=> {row[2]}, password=> {row[3]}, GPA=> {row[4]}, group=>{row[5]})""")
...
def REGISTER_COURSES(id):
    ...
    print("\n")
    while True:
        code = input("Enter the code of course you want to register: ")
...
def CHOOSE_GROUP(id):
    ...
    d = cursor.fetchone()
    if d[5] != ' ':
        print("You Have Chose your Group Before")
        return
    while True:
...
Who are you?, enter your choice : """).lower()
    ...
    if user_choice == "control":
        ...
        while True:
            ...
            while control_choice not in ["1", "2", "3", "4", "5", "6", "7", "8"]:
                print(f"This value \"{control_choice}\" is not exists, enter a valid value ")
        ...
        while True:
            ...
            while student_choice not in ["1", "2", "3", "4", "5"]:
                print(f"This value \"{student_choice}\" is not exists, enter a valid value ")
        ...
        print("error, This value is not exists")
    db.close()
